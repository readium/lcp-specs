...
| Server side behavior | HTTP Status Code | Client side behavior |
| -------------------- | ---------------- | -------------------- | 
| The server <b class="rfc">must</b> ensure that the `end` date and time of the license is updated. The `end` date and time <b class="rfc">may<b> be set to the new expiration date for the license, or it <b class="rfc">may<b> correspond to an extension period controlled by the server.<br /> The server <b class="rfc">must</b> return an updated Status Document. <br /> The server <b class="rfc">must</b> update the timestamp of both License and Status Document contained in the `status` and `license` keys of the `updated` object.<br /> The server <b class="rfc">may</b> also add a new event in the `events` object of the Status Document. | 200 | The client <b class="rfc">must</b> process the updated Status Document.<br /> The client <b class="rfc">must</b> download the updated License Document.<br /> The client <b class="rfc">may</b> attempt to renew the License again later. |
